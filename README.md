...
Green Roots is a web application designed to encourage tree planting and promote environmental protection. This app allows users to participate in tree planting activities, earn points, connect with community events, compete through rankings, redeem rewards, provide feedback, manage account settings, update profile pictures, configure payment methods (prototype), and securely log out, all contributing to a greener planet. It is built with a user-friendly interface to make it accessible to everyone, even those unfamiliar with technology.
...
Cash Withdrawal: Points convert to cash (1 point = ₱0.50, minimum 100 points) via PayPal, limited to 5 withdrawals per hour. Requires a PayPal email set in Payment Methods (see Payment Methods).
...
Updating PayPal Email: The “Payment Methods” page displays the current PayPal email (if set) and offers a form to update it. Users enter a new email, which is required for cash withdrawals in the Rewards feature (see Rewards).
...
views/: Page templates (e.g., login.php, events.php, feedback.php, account_settings.php, profile.php, payment_methods.php, logout.php).
...
The app uses a MySQL database with the following key tables:
...
Columns: user_id (PK, int), username (varchar, unique), email (varchar, unique), password (varchar), phone_number (varchar, nullable), first_name (varchar), last_name (varchar), barangay_id (int, FK), profile_picture (blob, nullable), default_profile_asset_id (int, FK, nullable), paypal_email (varchar, nullable), created_at (timestamp).



barangays: Stores location data.





Columns: barangay_id (PK, int), name (varchar), city (varchar), province (varchar), region (varchar), country (varchar), coordinates (varchar, nullable).



submissions: Tracks tree planting submissions.





Columns: submission_id (PK, int), user_id (int, FK), num_trees (int), photo (blob), status (enum: pending, approved, rejected), submitted_at (timestamp).
...
Columns: event_id (PK, int), title (varchar), date (datetime), location (varchar), barangay_id (int, FK), capacity (int), created_at (timestamp).
...
Columns: participant_id (PK, int), user_id (int, FK), event_id (int, FK), qr_code (varchar), joined_at (timestamp), confirmed_at (timestamp, nullable).
...
Columns: activity_id (PK, int), user_id (int, FK), description (varchar), activity_type (enum: submission, event, reward, feedback), created_at (timestamp).
...
Columns: feedback_id (PK, int), user_id (int, FK), category (enum: bug, feature, general), rating (int, 1-5), comments (text, max 1000 chars), is_anonymous (boolean), status (enum: submitted, under_review, resolved), submitted_at (timestamp), response (text, nullable).



assets: Stores images (e.g., favicon, logo, profile pictures).





Columns: asset_id (PK, int), asset_type (varchar, e.g., favicon, logo, default_profile), asset_data (blob).
...
Create database greenroots_db in phpMyAdmin.



Execute SQL to create tables (see schema above).



Configure includes/config.php with database credentials (e.g., host: localhost, username: root, password: '').
...
Set Up Server: Install XAMPP, start Apache and MySQL.



Create Database: In phpMyAdmin, create greenroots_db and run SQL from Database Setup.



Configure: Edit includes/config.php with database credentials (default: username “root”, no password).



Access: Open http://localhost/green-roots/index.php.
...
Prototype MVC-like structure:





Model: MySQL database, PHP logic (config.php, services).



View: HTML templates (events.php, feedback.php, account_settings.php, profile.php, payment_methods.php).



Controller: PHP scripts for user inputs and database interactions.

Future plans: Full MVC framework, API, or new backend technologies.
...
Includes sidebar, search bar, notifications, profile dropdown with links to Account Settings, Profile, Payment Methods, and Logout.
